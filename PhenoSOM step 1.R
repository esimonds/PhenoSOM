# PhenoSOM of viable-gated single-cell data
# Author:  Erin Simonds
# Version: 1.210521
#
# Motivation: This script ("PhenoSOM") links together FlowSOM (Van Gassen et al, Cytometry A, 2015)
# and PhenoGraph (Levine et al, Cell, 2015) as implemented in CytofKit (Chen et al, PLoS Comp Biol 2016).
# The motivation was to find something faster than running PhenoGraph on each file individually
# and then again in a metaclustering step (as done in Levine et al 2015). FlowSOM is very fast and
# can also be used to produce metaclusters (as described in Van Gassen et al 2015), but I was not
# satisfied with the metaclusters it produced in complex datasets of tumor-infiltrating leukocytes.
# This combination of FlowSOM followed by PhenoGraph seems to be a good trade-off of speed and quality.
# 
# Summary: The script attempts to identify communities across many grids of SOM (Self-Organizing Map)
# nodes using Louvain Community Detection with Jaccard coefficients (PhenoGraph). The "overclustering"
# with FlowSOM is intentional as the PhenoGraph step will consolidate these excess cluster later.
# This script does a good job of segregating tumor cells from major immune subsets (e.g. lymphoid,
# myeloid) but a poor job of detecting finer immune subsets (e.g. Tregs, dendritic cells). For the
# latter, it is recommended to run the script "PhenoSOM CD45-positive metaclusters" after this one.
#
# Some key parameters can be modified:
#
# 1. somx=30 and somy=30
# The default SOM grid is 30 x 30 = 900 nodes. This has worked well for me with datasets containing about
# 40 FCS files and about 10 million cells. I don't know how this would perform on much larger or smaller datasets.
# You can make the SOM map bigger or smaller by modifing xdim and ydim.
#
...
#     Transform the flowSet with asinh
#     Assign each FCS file a text GUID and a numerical GUID. Store this in a key table.
...
# Tell the script where to find the FCS files
FCSdir <- "renamed_FCSfiles"
FCSfilenamepattern=".*\\.fcs"

# Define the size of the SOM map (see notes above for more details)
somx = 30
somy = 30

# Set a project name for this run of PhenoSOM
projectname <- "your project name PhenoSOM Step 1"

outdir <- "PhenoSOM_Step1_output"
